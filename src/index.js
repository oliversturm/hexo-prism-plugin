...
const getPrism = require("./node-prismjs");
...
const extraLangs = hexo.config.prism_plugin.extraLangs || [];
...
function importAssets(code, data) {
  ...
  if (line_number && custom_css === null) {
    css.push(
      `<link rel="stylesheet" href="${rootPath}css/prism-line-numbers.css" type="text/css">`
    );
  }
  if (mode === "realtime") {
    ...
    if (line_number) {
      js.push(
        `<script src="${rootPath}js/prism-line-numbers.min.js"></script>`
      );
    }
  }
